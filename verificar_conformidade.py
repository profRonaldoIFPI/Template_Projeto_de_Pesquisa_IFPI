...
Template de Projeto de Pesquisa - IFPI
...
à NBR 15287 (Projeto de Pesquisa) e às normas ABNT vigentes.
...
def print_header():
    print(f"\n{BLUE}{BOLD}" + "="*72)
    print("   VERIFICADOR DE CONFORMIDADE DE PROJETO DE PESQUISA (IFPI)")
    print("   Conforme Manual do IFPI (2024), NBR 15287 e Normas ABNT Vigentes")
    print("="*72 + f"{RESET}\n")
...
def run_audits():
    ...
    # --------------------------------------------------------------------------
    # 1. Margens da Folha (Superior/Esquerda 3cm, Inferior/Direita 2cm)
    # --------------------------------------------------------------------------
    margin_ok = False
    clean_tex = all_tex.replace(" ", "")
    if ("left=3cm" in clean_tex or "left=3.0cm" in clean_tex or "\\setlrmarginsandblock{3cm}{2cm}{*}" in clean_tex) and \
       ("right=2cm" in clean_tex or "right=2.0cm" in clean_tex) and \
       ("top=3cm" in clean_tex or "top=3.0cm" in clean_tex or "\\setulmarginsandblock{3cm}{2cm}{*}" in clean_tex) and \
    ...
    results.append({
        ...
        "status": margin_ok,
        "msg": "Margens configuradas corretamente conforme ABNT NBR 14724 / IFPI." if margin_ok else "Verifique as configurações de margem no config.tex."
    })

    # --------------------------------------------------------------------------
    # 2. Tipografia Uniforme (Arial / helvet ou Times New Roman / mathptmx)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # 3. Indentação de Primeira Linha do Parágrafo (1,25 cm)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    capa_ok = "\\imprimircapa" in pre_textuais or "\\imprimircapa" in main_tex
    folha_rosto_ok = "\\imprimirfolhaderosto" in pre_textuais or "\\imprimirfolhaderosto" in main_tex
    ...
    results.append({
        ...
        "msg": "Capa e Folha de Rosto incluídas conforme o Manual do IFPI (2024)." if pre_elements_ok else "O Manual do IFPI exige a inclusão de Capa e Folha de Rosto para o Projeto de Pesquisa."
    })
    ...
    # --------------------------------------------------------------------------
    clean_code = "\n".join([line for line in main_tex.split("\n") if not line.strip().startswith("%")])
